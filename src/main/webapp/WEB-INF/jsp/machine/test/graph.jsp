...
<script type="text/javascript">
	window.onload = function() {
		...
		var chart = new CanvasJS.Chart("chartContainer", {
			title : {
				text : "[ ${room.roomName} ]에 대한 통계 <미세먼지 - 온도 - 습도>" 
			},
			data : [ {
				type : "line",

				dataPoints : [  {
					x : parseFloat("${totalPm/cnt}"),
					y : parseFloat("${totalPm/cnt}")
				}, {
					x : parseFloat("${totalTemperature/cnt}"),
					y : parseFloat("${totalTemperature/cnt}")
				}, {
					x :parseFloat("${totalHumadity/cnt}"),
					y : parseFloat("${totalHumadity/cnt}")
				} ]
